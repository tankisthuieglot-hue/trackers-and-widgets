...
![Trackers & Widgets collection](docs/media/pack.png)

## Download

| Language | Stable bundle | Includes |
|---|---|---|
| English | [Download `trackers-and-widgets-en.zip`](https://github.com/tankisthuieglot-hue/trackers-and-widgets/releases/latest/download/trackers-and-widgets-en.zip) | RPG HUD, Dice, Achievement, Clock, Relationship, Critics, Encounter Dossier, System Core, Moment Watch, shared service regexes |
| Russian | [Download `trackers-and-widgets-ru.zip`](https://github.com/tankisthuieglot-hue/trackers-and-widgets/releases/latest/download/trackers-and-widgets-ru.zip) | RPG HUD, Dice, Achievement, Clock, Relationship, Critics, Encounter Dossier, System Core, Moment Watch, shared service regexes |
...
An opt-in RPG System for roleplays that explicitly maintain levels, XP,
inventory, skills, and objective conditions. Invisible sync markers carry the
latest complete snapshot; visible events appear only for XP, loot, skills,
level-ups, status changes, or system notices.

The compact signal unfolds into one physical console with a mechanical XP core,
glass inventory drawer, elemental skill constellation, and diagnostic status
scanner. Each event has its own ritual, tabs act as tactile modules,
model-provided data opens as nested mini-widgets, and the three material layers
respond with pointer parallax. The central eye keeps breathing and blinking
after streamed responses finish.

![System Core animation](docs/media/system-en.gif)

Folder: [`dist/system/`](dist/system/)

### Moment Watch

Appears only when the story establishes an exact in-world time or makes a
meaningful time jump. It records the hour, minute, optional date, transition,
and one observable detail without inventing a clock merely to show the widget.

The pocket watch sets itself mechanically after streaming finishes: four
number drums roll into place, the minute hand makes a full setting revolution,
the hour hand follows, and the crown winds without detaching from the case.
Its physical day-phase disc distinguishes dawn, day, dusk, and night. Tapping
the glass opens the lid in 3D and reveals the recorded moment engraved inside:

![Moment Watch animation](docs/media/time-en.gif)

Folder: [`dist/time/`](dist/time/)

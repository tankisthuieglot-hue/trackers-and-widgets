...
Сборка кладёт в `dist/wallet/<язык>/` три файла: промпт для пресета, регекс и
чистилку. Промпт и регекс берутся из одного описания полей, поэтому обещать
поле, которого регекс не ловит, физически невозможно.
